...
export async function makeNewFilter() {
	...
	if (!cont.length) toast({ html: `Error:${lang.lang_filter_errordegree}` })
	const exc = !!$('#except_filter:checked').val()
	const who = !!$('#wholeword_filter:checked').val()
	const time =
		parseInt($('#days_filter').val()?.toString() || '0', 10) * 24 * 60 * 60 +
		parseInt($('#hours_filter').val()?.toString() || '0', 10) * 60 * 60 +
		parseInt($('#mins_filter').val()?.toString() || '0', 10) * 60
	const domain = localStorage.getItem('domain_' + acctId)
	const at = localStorage.getItem('acct_' + acctId + '_at')
...
export function notfFilter(id: string, tlid: string) {
